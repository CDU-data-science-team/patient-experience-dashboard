...
Please note further that Patient Opinion data is imperfectly matched from the Patient Opinion database and the numbers within this report may differ substantially from the true numbers (especially at team level). Patient Opinion story numbers should be read as minima, not maxima.
...
names(trustTable) = c("SUCE responses", "PO responses")
...
rownames(combTable) = paste0(rep(c("SUCE responses- ", "PO responses- "), each = 3),
...
# for(i in c(16)){
  ...
  names(dirTable) = c("SUCE responses", "PO responses")
  ...
  if(nrow(poTab) > 0){
    ...
    names(shapePO) = c("Team", paste0("PO: ", timetext[Quarter : (Quarter - 4)]))
    ...
    cat("<br>\n\n")
      ...
      cat("No Patient Opinion stories matched to team in the previous year\n  <br/>")
    }
